...
@section('content')
    <flux:main>
        <flux:heading size="xl" level="1">Index Jenis Ruangan</flux:heading>
        ...
        <div class="flex justify-end mb-4 gap-4">
            <div>
                ...
                <form action="{{ route('jenis-ruangan.store') }}" method="post">
                    ...
                    <flux:input label="Jenis Ruangan" type="text" name="name" placeholder="Nama jenis ruangan" required />
        ...
        @if(count($daftar_jenis_ruangan) > 0)
            ...
            @foreach ($daftar_jenis_ruangan as $jenis_ruangan)
                <tr class="border-b">
                    <td contenteditable="true" class="p-3">{{ $jenis_ruangan->name }}</td>
                    <td class="p-3">
                        <div class="flex justify-center items-center space-x-2">
                            <div>
                                ...
                                @endslot
                                @slot('triggerModal') modal-edit-{{ $jenis_ruangan->id }} @endslot
                                @slot('modalInputForm')

                                <div>
                                    <flux:heading size="lg">Edit Data</flux:heading>
                                    <flux:text class="mt-2">Edit jenis ruangan.</flux:text>
                                </div>
                            ...
                            <div>
                                ...
                                @slot('triggerName') modal-delete-{{ $jenis_ruangan->id }} @endslot
                                @slot('url') {{ route('jenis-ruangan.delete', $jenis_ruangan->id) }} @endslot
                                @endcomponent
                            </div>
                        </div>
                    </td>
        ...
        @else
            ...
            <tr class="py-6">
                <td class="py-5 border-gray-200 p4" colspan="3">
                    <h4 class="text-center">Data jenis ruangan kosong</h4>
                </td>
            </tr>
